 ...
 * @internal    @properties &menuindex=Позиция виджета;int;100;0 &id_help=ID документа помощи;int;2;1 &css=CSS;textarea;; &js=JS;textarea;;
...
switch($e->name){
	case 'OnManagerWelcomeHome': 
		...
		if($modx->db->getRecordCount( $result ) == 1):
			...
			$help_output .=  '<div class="card-body"><div id="help_content"><div id="frame_help">' . $row["content"] . '</div></div></div>';
			$widgets['help-widget'] = array(
				...
				'title' => $help_title,
				'body' => $help_output . '<style>.list-decimal > li {list-style: decimal;} h3, .h3 {color: red;}' . $css . '</style><script>' . $js . '</script>'
			);
		endif;
